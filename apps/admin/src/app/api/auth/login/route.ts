...
export async function POST(request: NextRequest) {
  ...
  if (!loginResponse.ok) {
    return NextResponse.json(loginData ?? { message: 'Login failed.' }, { status: loginResponse.status })
  }

  const token = loginData?.token || loginData?.access_token

  if (!token) {
    return NextResponse.json({ message: 'Login response did not contain an access token.' }, { status: 502 })
  }
